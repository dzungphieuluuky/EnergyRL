...
def calculate_path_loss(distance, frequency_hz, ue_id, current_time, seed):
    ...
    """
    # Set seed for reproducibility, matching MATLAB's stream logic
    ...
    hBS = 25.0  # Base station height (fixed, as in MATLAB)
    hUT = 1.5   # User terminal height (fixed, as in MATLAB)
    
    # Line-of-Sight (LOS) probability calculation
    if distance <= 18.0:
        pLOS = 1.0
    else:
        pLOS = 18.0 / distance + math.exp(-distance / 36.0) * (1.0 - 18.0 / distance)
    
    # Determine LOS/NLOS based on probability
    if np.random.random() < pLOS:
        # LOS path loss formula
...
def calculate_rsrp_batch(ue_positions, cell_positions, tx_powers, frequency, 
    ...
    Matches the measurement loop in MATLAB's updateSignalMeasurements.
    """
    num_ues = ue_positions.shape[0]
    num_cells = cell_positions.shape[0]
    rsrp_matrix = np.zeros((num_ues, num_cells), dtype=np.float64)
    
    for ue_idx in prange(num_ues):  # Parallelize over UEs for efficiency
        ue_x, ue_y = ue_positions[ue_idx]
        ue_id = ue_ids[ue_idx]
        
        for cell_idx in range(num_cells):  # Sequential over cells to avoid race conditions
            ...
            # Get path loss for this specific UE-cell pair
            path_loss = calculate_path_loss(distance, frequency, ue_id, current_time, seed)
            
            # RSRP calculation with measurement variation
            ...
            # Power penalty for low transmit power, matching MATLAB logic
...
def process_measurements_batch(rsrp_matrix, tx_powers, min_tx_powers, 
    ...
    Matches the RSRQ/SINR calculation logic in MATLAB's updateSignalMeasurements.
    """
    ...
    for ue_idx in prange(num_ues):
        for cell_idx in range(num_cells):
            ...
            if rsrp >= (rsrp_measurement_threshold - 5.0):
                ...
                # SINR calculation (noise floor at -110 dBm)
                sinr = rsrp - (-110.0) + (np.random.randn() * 2.0)
                
                # SINR penalty for low power (6dB penalty as per MATLAB)
...
def evaluate_handover_success_prob(serving_tx_power, serving_min_power, 
    ...
    Matches MATLAB evaluateHandoverSuccess logic exactly.
    """
    ...
    # Signal quality bonus/penalty based on RSRP
    if neighbor_rsrp >= -75:     signal_bonus = 0.02
    elif neighbor_rsrp >= -85:   signal_bonus = 0.01
    elif neighbor_rsrp >= -95:   signal_bonus = 0.0
    elif neighbor_rsrp >= -105:  signal_bonus = -0.05
    else:                        signal_bonus = -0.15
    
    # SINR bonus/penalty
    if neighbor_sinr >= 15:      sinr_bonus = 0.02
    elif neighbor_sinr >= 5:     sinr_bonus = 0.01
    elif neighbor_sinr >= 0:     sinr_bonus = 0.0
    elif neighbor_sinr >= -5:    sinr_bonus = -0.03
    else:                        sinr_bonus = -0.10
    
    # Penalty if both cells are at minimum power
    if (serving_tx_power <= serving_min_power + 1 and 
        target_tx_power <= target_min_power + 1):
        base_success_prob -= 0.20
    
    # Congestion penalty for the target cell
    if target_cpu_usage > 85 or target_prb_usage > 85:
        congestion_penalty = 0.05
        base_success_prob -= congestion_penalty
    
    # Final probability calculation, clamped between 25% and 98%
    final_success_prob = base_success_prob + signal_bonus + sinr_bonus
    return max(0.25, min(0.98, final_success_prob))
...
def calculate_drop_probability(cell_tx_power, cell_min_power, cell_max_power,
    ...
    # Power penalty - increases drop probability at lower power levels
    power_range = cell_max_power - cell_min_power
    if cell_tx_power <= cell_min_power + power_range / 4.0:
        power_penalty = (cell_min_power + power_range / 4.0 - cell_tx_power) * 0.08
        drop_prob += power_penalty
    elif cell_tx_power <= cell_min_power + power_range / 2.0:
        power_penalty = (cell_min_power + power_range / 2.0 - cell_tx_power) * 0.03
        drop_prob += power_penalty
    
    # Signal quality factor - SINR
    if not np.isnan(ue_sinr):
        if ue_sinr < -10:   drop_prob += 0.15 + abs(ue_sinr + 10) * 0.02
        elif ue_sinr < -5:  drop_prob += 0.08 + abs(ue_sinr + 5) * 0.015
        elif ue_sinr < 0:   drop_prob += 0.04
        elif ue_sinr < 5:   drop_prob += 0.01
    
    # Signal quality factor - RSRP
    if not np.isnan(ue_rsrp):
        if ue_rsrp < -120:  drop_prob += 0.12 + abs(ue_rsrp + 120) * 0.01
        elif ue_rsrp < -115:drop_prob += 0.06 + abs(ue_rsrp + 115) * 0.008
        elif ue_rsrp < -110:drop_prob += 0.03
    
    # Cell congestion factor - CPU
    if cell_cpu_usage > 95:     drop_prob += (cell_cpu_usage - 95) * 0.015
    elif cell_cpu_usage > 90:   drop_prob += (cell_cpu_usage - 90) * 0.01
    
    # Cell congestion factor - PRB
    if cell_prb_usage > 95:     drop_prob += (cell_prb_usage - 95) * 0.012
    elif cell_prb_usage > 90:   drop_prob += (cell_prb_usage - 90) * 0.008
    
    # Traffic load factor
    load_ratio = cell_current_load / max(1.0, cell_max_capacity)
    if load_ratio > 0.98:       drop_prob += (load_ratio - 0.98) * 1.0
    elif load_ratio > 0.95:     drop_prob += (load_ratio - 0.95) * 0.6
    elif load_ratio > 0.90:     drop_prob += (load_ratio - 0.90) * 0.2
    
    # Critical condition penalty (low signal despite high power)
    if cell_tx_power >= cell_max_power and (ue_rsrp < -110 or ue_sinr < -5):
        drop_prob += 0.25
    
    # Clamp to a maximum probability of 45%
    return min(0.45, drop_prob)
...
def calculate_cell_cpu_usage(power_ratio, num_ues, load_ratio):
    ...
    return min(95.0, base_cpu + per_ue_cpu + load_cpu)
...
def calculate_cell_prb_usage(num_ues, load_ratio):
    ...
    return min(95.0, base_prb + load_prb)


@jit(nopython=True, cache=True)
def calculate_cell_energy_consumption(base_energy, idle_energy, tx_power, num_ues, load_ratio):
    """
    Calculate total cell energy consumption.
    Matches MATLAB updateCellResourceUsage.
    """
    # Convert dBm to watts for power consumption calculation
    tx_power_watts = 10.0 ** ((tx_power - 30.0) / 10.0)
    per_ue_energy = num_ues * 15.0
    load_energy = load_ratio * 200.0
    # MATLAB logic uses baseEnergy for the full calculation, not idle_energy
    return base_energy + tx_power_watts + per_ue_energy + load_energy
...
def calculate_theoretical_drop_rate(base_drop_rate, tx_power, min_tx_power, 
    ...
    # Power drop penalty
    power_range = max(1e-6, max_tx_power - min_tx_power)
    if tx_power <= min_tx_power + power_range / 4.0:
        power_drop_penalty = (min_tx_power + power_range / 4.0 - tx_power) * 4.0
        drop_rate += power_drop_penalty
    elif tx_power <= min_tx_power + power_range / 2.0:
        power_drop_penalty = (min_tx_power + power_range / 2.0 - tx_power) * 1.5
        drop_rate += power_drop_penalty
    
    # Congestion factors - CPU
    if cpu_usage > 90:      drop_rate += (cpu_usage - 90) * 0.4
    elif cpu_usage > 85:    drop_rate += (cpu_usage - 85) * 0.2
    
    # Congestion factors - PRB
    if prb_usage > 90:      drop_rate += (prb_usage - 90) * 0.3
    elif prb_usage > 85:    drop_rate += (prb_usage - 85) * 0.15
    
    # Signal quality factor
    if valid_sinr_count > 0:
        if avg_sinr < 0:    drop_rate += abs(avg_sinr) * 0.2
        elif avg_sinr < 5:  drop_rate += (5 - avg_sinr) * 0.1
    
    # Clamp to maximum 25%
    return min(25.0, drop_rate)
...
def calculate_cell_latency(base_latency, load_ratio, num_ues, 
    ...
    power_range = max(1e-6, max_tx_power - min_tx_power)
    if tx_power <= min_tx_power + power_range / 4.0:
        power_latency_penalty = (min_tx_power + power_range / 4.0 - tx_power) * 15.0
    elif tx_power <= min_tx_power + power_range / 2.0:
        power_latency_penalty = (min_tx_power + power_range / 2.0 - tx_power) * 8.0
    else:
